...
MGMT_WORKER_BIN_PYTHON = '/opt/mgmtworker/env/bin/python'
...
is_install = not is_upgrade and not is_rollback


def get_influxdb_current_port():
    es_props = ctx_factory.load_install_properties('influxdb')
    return es_props['influxdb_endpoint_port']
...
class CtxPropertyFactory(object):
    ...
    def _load_ctx_properties(self, service_name):
        ...
        if self.is_upgrade:
            ...
            if use_existing:
                existing_props = self.load_install_properties(service_name)
    ...
    def load_install_properties(self, service_name):
        install_props_path = \
...
class BlueprintResourceFactory(object):
    ...
    def archive_resources(self, service_name):
        ...
        if self.is_upgrade and os.path.isdir(service_archive_path):
